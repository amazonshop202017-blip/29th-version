...
export const AddWidgetPlaceholder = ({ onClick, size = 'md', className = '', variant = 'edit' }: AddWidgetPlaceholderProps) => {
  ...
  const tone = variant === 'subtle'
    ? 'border-muted-foreground/20 bg-transparent hover:bg-muted/30 hover:border-primary/40'
    : 'border-muted-foreground/30 bg-muted/20 hover:bg-muted/40 hover:border-primary/40';
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-2 ${minH} h-full w-full rounded-xl border-2 border-dashed ${tone} transition-all cursor-pointer group ${className}`}
    >
      ...
      <span className={`${label} text-muted-foreground/60 group-hover:text-muted-foreground transition-colors`}>
        {variant === 'subtle' ? 'Add widget' : 'Click to add widget'}
      </span>
    </button>
  );
};
