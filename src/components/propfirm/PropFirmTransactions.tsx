  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
      ...
      <div className="bg-white rounded-xl border border-border shadow-sm p-4">
        ...
        <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-emerald-50 flex items-center justify-center shrink-0"><TrendingUp className="w-4 h-4 text-emerald-500" /></div><div><div className="text-2xl font-bold text-emerald-600 tracking-tight">+$500</div><div className="text-xs text-muted-foreground mt-0.5">Payouts &amp; commissions received</div></div></div>
      </div>
      <div className="bg-white rounded-xl border border-border shadow-sm p-4">
        <div className="text-xs text-muted-foreground font-medium mb-3">Total spent</div>
        <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-rose-50 flex items-center justify-center shrink-0"><TrendingDown className="w-4 h-4 text-rose-500" /></div><div><div className="text-2xl font-bold text-rose-500 tracking-tight">-$167</div><div className="text-xs text-muted-foreground mt-0.5">$167 in eval fees</div></div></div>
      </div>
      <div className="bg-white rounded-xl border border-border shadow-sm p-4">
        <div className="text-xs text-muted-foreground font-medium mb-3">Net cash flow</div>
        <div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><DollarSign className="w-4 h-4 text-primary" /></div><div><div className="text-2xl font-bold text-emerald-600 tracking-tight">+$333</div><div className="text-xs text-muted-foreground mt-0.5">Income minus expenses</div></div></div>
      </div>
    </div>
  );
}
...
function TypeBadge({ type }: { type: "Income" | "Expense" }) {
  return <span className={`inline-flex items-center text-[11px] font-medium px-2 py-0.5 rounded-full ${type === "Income" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-600"}`}>{type}</span>;
}
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
        <div className="flex items-center gap-0 bg-white border border-border rounded-lg p-0.5 overflow-x-auto shrink-0">
          {filterTabs.map((tab) => (<button key={tab} onClick={() => setFilterTab(tab)} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors whitespace-nowrap ${filterTab === tab ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}>{tab}</button>))}
        </div>
        <div className="flex items-center gap-2 flex-1 sm:justify-end">
          <div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" /><input type="text" placeholder="Search by firm, challenge, account, or category" className="w-full pl-9 pr-3 py-2 text-xs border border-border rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-primary/30 placeholder:text-muted-foreground/60" readOnly /></div>
          <button className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium border border-border rounded-lg bg-white text-muted-foreground hover:text-foreground transition-colors"><SlidersHorizontal className="w-3.5 h-3.5" />Filters</button>
        </div>
      </div>
      <div className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-b border-border">
          ...
          <button className="px-3 py-1.5 text-xs font-medium border border-border rounded-lg text-foreground bg-white hover:bg-muted/30 transition-colors">+ Add transaction</button>
        </div>
            ...
            <tbody>
              {transactions.map((tx) => (
                <tr key={tx.id} className="border-b border-border last:border-0 hover:bg-muted/10 transition-colors">
                  ...
                  <td className="px-3 py-3.5 text-right whitespace-nowrap"><span className={`text-sm font-semibold ${tx.amount > 0 ? "text-emerald-600" : "text-rose-500"}`}>{tx.amount > 0 ? `+$${tx.amount.toFixed(2)}` : `-$${Math.abs(tx.amount).toFixed(2)}`}</span></td>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border">
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><span>Transactions per page:</span><button className="flex items-center gap-1 border border-border rounded-md px-2 py-1 text-foreground text-xs bg-white">10<ChevronDown className="w-3 h-3" /></button></div>
